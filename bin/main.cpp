...
int main(int argc, char *argv[])
{
    ...
    while (gladiators.size() > 1)
    {
        for (int i = 1; i < gladiators.size(); i += 2)
        {
            ...
            std::cout << "\033[1;34m";
            std::cout << "!!! " << gladiators[i - 1]->GetName() << " VS " << gladiators[i]->GetName() << " !!!\n";
            std::cout << "\033[0m";
            std::cin.get();

            while (firstWins < ROUNDS_AMOUNT && secondWins < ROUNDS_AMOUNT && draws < DRAW_MAX)
            {
                ...
                else if (choiseFirst - choiseSecond == 1 || choiseFirst - choiseSecond == -2)
                {
                    firstWins++;
                    draws = 0;
            ...
            if (firstWins == ROUNDS_AMOUNT)
            {
                ...
                std::cout << "\033[1;34m";
                std::cout << gladiators[i - 1]->GetName() << " wins:" << gladiators[i - 1]->GetWinPhrase() << '\n';
                std::cout << gladiators[i]->GetName() << " looses:" << gladiators[i]->GetLosePhrase() << '\n';
                std::cout << "\033[0m";
            ...
            else if (secondWins == ROUNDS_AMOUNT)
            {
                ...
                std::cout << "\033[1;34m";
                std::cout << gladiators[i]->GetName() << " WINS: " << gladiators[i]->GetWinPhrase() << '\n';
                std::cout << gladiators[i - 1]->GetName() << " LOOSES: " << gladiators[i - 1]->GetLosePhrase() << '\n';
            ...
            {
                std::cout << "\033[1;34m";
                std::cout << "No one wants to fight seriously... Both players terminated!!!\n";
                std::cout << "\033[0m";
                gladiators.erase(gladiators.begin() + i);
                gladiators.erase(gladiators.begin() + i - 1);
            }
